...
sys.path.append(
    "../.."
)  # There are some funky, circular dependencies that will need to be cleaned up
...
def run_chronux(spike_times, spike_clusters, cluster_ids, x, xt, t0, tf, verbose=True):
    ...
    data_out = {}
    data_out["x"] = x
    data_out["spike_times"] = (
        spike_times - t0
    )  # Must subtract t0 here in order to align the first sample
...
def reshape_chronux_output(
    chronux_rez, cluster_ids, n_total_clusters, mode="unit_level", min_rr=0.2
):
    ...
    if mode == "mat":
        ...
        save_data["full_coherence_lb"] = chronux_rez["full_coherence_lb"][
            :, :chop_freqs_idx
        ]
        save_data["full_coherence_ub"] = chronux_rez["full_coherence_ub"][
            :, :chop_freqs_idx
        ]
        ...
        unit_level_coh.loc[cluster_ids, "coherence"] = chronux_rez["full_coherence"][
            :, rr_sample
        ]
        unit_level_coh.loc[cluster_ids, "coherence_lb"] = chronux_rez[
            "full_coherence_lb"
        ][:, rr_sample]
        unit_level_coh.loc[cluster_ids, "coherence_ub"] = chronux_rez[
            "full_coherence_ub"
        ][:, rr_sample]
        unit_level_coh.loc[cluster_ids, "phi_std"] = chronux_rez["full_phistd"][
            :, rr_sample
        ]
        unit_level_coh.loc[cluster_ids, "phi"] = chronux_rez["full_phi"][:, rr_sample]
        unit_level_coh.loc[cluster_ids, "cluster_id"] = cluster_ids
        unit_level_coh.loc[cluster_ids, "phi_adj"] = adjust_chronux_phi(
            unit_level_coh["phi"]
        )
...
def run_phy_probe(phy_path, t0, tf, x, xt, use_good=True, verbose=True):
    ...
    # Run Chronux
    chronux_rez = run_chronux(
        spike_times, spike_clusters, cluster_ids, x, xt, t0, tf, verbose=verbose
    )

    # Shape results
    unit_coh = reshape_chronux_output(
        chronux_rez, cluster_ids, n_total_clusters, mode="unit_level"
    )
...
def run_session(session_path, t0, tf, var="dia", use_good=True, verbose=True):
    ...
    physiology_data = alfio.load_object(
        session_path.joinpath("alf"), "physiology", short_keys=True
    )
...
@click.option(
    ...
    default=0,
    show_default=True,
    help="Time in seconds of the beginning of the window to consider in the coherence computation.",
)
@click.option(
    "--tf",
    default=300,
    show_default=True,
    help="Time in seconds of the end of the window to consider in the coherence computation.",
)
@click.option(
    "--var",
    default="dia",
    show_default=True,
    help="Which variable to compute coherence against",
)
@click.option(
    "--include_all",
    "-i",
    is_flag=True,
    help='If set, runs on all units. Default behavior is to run only on the units identified as "good"',
)
@click.option(
    "--verbose", "-v", is_flag=True, help="Updates user of computation for each unit"
)
