...
class Recording:
    ...
    def concatenate_triggers(self, object_name):
        try:
            alf_obj_out = alfio.load_object(
                self.alf_path, object_name, extra=f"t0", short_keys=True
            )
        except:
    ...
    def concatenate_log(self, save=True, overwrite=True):
        try:
            alf_obj_out = alfio.load_object(
                self.session_path, "log", extra=f"t0", short_keys=True
            )
    ...
    def concatenate_alf_objects(self, save=True, overwrite=True):
        for object_name in self.alf_objects:
            if object_name == "log":
                _log.debug(
                    f"Do not concatenate log here - it requires special handling"
                )
                continue
